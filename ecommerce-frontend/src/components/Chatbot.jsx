...
const Chatbot = ({ onOpenChange }) => {
    ...
    useEffect(() => {
        // Restore scroll when chatbot is closed
        if (!isOpen) {
            document.body.style.overflow = 'auto';
        }
    }, [isOpen]);
    ...
    return (
        <>
            ...
            <button
                ...
                title={isOpen ? 'Close Chat' : 'Chat with Basket AI'}
            >
                {isOpen ? (
            ...
            <div 
                ...
                className={`fixed bottom-28 left-4 z-50 w-[350px] sm:w-[380px] transition-all duration-500 ease-in-out ${
                    isOpen
                ...
                <div className={`rounded-3xl shadow-2xl overflow-hidden border flex flex-col h-[520px] ${
                            ...
                            <div key={index} className={`flex items-start gap-2.5 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div className={`max-w-[75%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                                    msg.role === 'user'
                                        ...
                                        : (darkMode ? 'bg-gray-800 text-gray-200 rounded-tl-sm border border-gray-700' : 'bg-blue-50 text-gray-800 rounded-tl-sm border border-blue-100')
                                }`}>
                                    {formatText(msg.text)}
                                </div>
                            </div>
                                ...
                                {QUICK_SUGGESTIONS.map((s, i) => (
                                    <button
                                        ...
                                        className={`text-xs px-3 py-1.5 rounded-xl border font-medium transition-all hover:scale-105 ${
                                            darkMode ? 'border-gray-700 text-blue-400 hover:bg-gray-800' : 'border-blue-200 text-blue-700 hover:bg-blue-50'
                                        }`}
                                    >
                                        {s}
                                    </button>
                    ...
                    <div className={`p-4 border-t ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
                        <div className={`flex items-center gap-2 rounded-2xl border px-4 py-2.5 transition-all ${
                            darkMode ? 'bg-gray-800 border-gray-700 focus-within:border-blue-500' : 'bg-gray-50 border-gray-200 focus-within:border-blue-400'
                        }`}>
                            <input
                                ref={inputRef}
                                type="text"
