...
const Register = () => {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        if (!role) { setMessage('Please select a role (Admin or Customer)'); return; }
    ...
    return (
        <div className="flex justify-center items-center min-h-[80vh] py-8">
            <div className="w-full max-w-md bg-white p-10 rounded-3xl shadow-2xl border border-blue-50">
                <div className="text-center mb-8">
                    <h2 className="text-4xl font-extrabold text-blue-900 mb-2">Create Account</h2>
                    <p className="text-blue-500 font-medium">Join our premium e-commerce platform</p>
                </div>

                {(error || message) && (
                    <div className="mb-5 bg-red-50 p-4 rounded-xl border border-red-200 text-red-600 text-center text-sm font-semibold">
                        {error || message}
                    </div>
                )}
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className={`py-3 px-4 rounded-xl border-2 font-semibold transition-all text-center ${role === 'admin' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-blue-300'}`}
                            >
                                🛠 Admin
                            </button>
                            <button
                                ...
                                className={`py-3 px-4 rounded-xl border-2 font-semibold transition-all text-center ${role === 'user' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-blue-300'}`}
                            >
                                🛒 Customer
                            </button>
                        </div>
                ...
                <div className="mt-6 text-center">
                    <p className="text-gray-600 text-sm">
                        Have an account?{' '}
                        <Link to="/login" className="text-blue-600 font-bold hover:text-blue-800 transition-colors">Login</Link>
                    </p>
                </div>
            </div>
        </div>
