...
// POST /api/seed — Seeds admin user + 18 products (only if products collection is empty)
router.post('/', async (req, res) => {
    try {
        const existingAdmin = await User.findOne({ email: 'ashfaaq@admin.com' });
        ...
        res.json({
            success: true,
            message: `Seeded admin + ${products.length} products`,
            adminEmail: 'ashfaaq@admin.com',
            adminPassword: 'ashfaaq123'
        });
    } catch (err) {
